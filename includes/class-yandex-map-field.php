...
class Yandex_Map_Field extends acf_field {
	...
	function enqueue_scripts() {
		...
		$lang_code = get_yandex_language_code();

		if ( $api_key ) {
			wp_enqueue_script(
				'yandex-maps-api',
				"https://api-maps.yandex.ru/2.1/?lang={$lang_code}&apikey={$api_key}",
				[],
				null
			);
		}
	...
	function render_field( $field ) {
      ...
      <div <?php echo acf_esc_attrs( $attrs ); ?> style="height: <?php echo esc_attr( $field['height'] ); ?>px;">
        ...
        <div class="yandex-map-coordinates">
          ...
          <input type="hidden"
                 name="<?php echo esc_attr( $field['name'] ); ?>[zoom]"
